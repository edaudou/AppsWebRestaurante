    ...
    <div id="reservaResults">
        <?php foreach ($viewmodel as $item) : ?>
            <div class="well">
                ...
                <a class="btn btn-primary" href="<?php echo ROOT_PATH; ?>reservas/edit?id=<?php echo $item['id']; ?>">Editar</a>
                <a class="btn btn-danger" href="<?php echo ROOT_PATH; ?>reservas/delete?id=<?php echo $item['id']; ?>" onclick="return confirm('¿Estás seguro de eliminar esta reserva?');">Eliminar</a>
            </div>
        <?php endforeach; ?>
    </div>
</div>
...
document.getElementById('searchReserva').addEventListener('keyup', function() {
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            let reservas = JSON.parse(xhr.responseText);
            let output = '';
